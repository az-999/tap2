...
import { ApiError, BuyRequest, getEnv, getJWTHash, getNftList, getTonClient, notifyTransactionStarted, sendError, validateHash } from "../utils/api";
...
export const routeTestBuy = async (req: Request, res: Response) => {
    try {
        res.set({
        ...
        if (token.substring(7) !== getEnv('BACKEND_TOKEN')) {
        ...
        console.log('Next IndexID: ', index);
        ...
        console.log('client_open.send(walletSender, { value: toNano(0.3), bounce: false }, MintMsg)'); //сама транза на минт
