...
export const setSecondMinter = async (client: TonClient4, address: Address, secondMinter: Address) => {
    ...
    // Parameters
    const msg: ShipApi.SetSecondMinter = {
        $$type: "SetSecondMinter",
        new_second_minter_address: secondMinter
    }
    ...
    const { body, queryId } = walletContract.createTransfer({
        ...
        messages: [internal({
            to: address,
            value: toNano('0.3'),
            body: beginCell().store(ShipApi.storeSetSecondMinter(msg)).endCell()
        })],
