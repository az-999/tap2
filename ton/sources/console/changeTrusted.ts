...
export const changeTrustedAddress = async (client: TonClient4, collection: string, address: string) => {
    ...
    const { body, queryId } = walletContract.createTransfer({
        ...
        messages: [internal({
            to: collection,
            value: "0.03",
            body: beginCell().store(ShipApi.storeChangeTreasury({ $$type: 'ChangeTreasury', new_treasury: Address.parse(address) })).endCell()
        })],
