...
import { bumpticketPrice } from "../utils/config";

export const deployBumpticketRouter = async (client4: TonClient4, refAddr: Address, prizeAddr: Address, mainAddr: Address) => {
    ...
    const stateInit = await BumpTicket.Router.init(owner.address, BigInt('0x' +owner.publicKey.toString('hex')), refAddr, prizeAddr, mainAddr, toNano(bumpticketPrice));
