import { Address, beginCell, Builder, Cell, contractAddress, StateInit } from 'ton'
import BN from 'bn.js'
import { NftFixPriceSaleV3CodeCell, NftFixPriceSaleV3R3CodeCell } from './NftFixpriceSaleV3.source'
...
export function buildNftFixPriceSaleV3R3StateInit(
  data: Omit<NftFixPriceSaleV3R3Data, 'isComplete'> | Omit<NftFixPriceSaleV3Data, 'isComplete'>
) {
  const dataCell = buildNftFixPriceSaleV3R3DataCell({
    ...data,
    // Nft owner address would be set by NFT itself by ownership_assigned callback
    isComplete: false,
  })
  ...
  const address = contractAddress({
    workchain: 0,
    initialCode: NftFixPriceSaleV3R3CodeCell,
    initialData: dataCell,
  })
