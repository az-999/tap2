            ...
            <article class="main__content-item main__content-form main__content-form_type_task-edit col col-xl-8">
                ...
                <div class="main__content-item-section">
                    ...
                    <form method="post">
                        ...
                        <div class="group" style="margin-top: 20px;">
                            <label>Награда *</label>
                            <input name="amount" class="form-control" value="{{$model->amount}}">
                        </div>
                        <div class="group" style="margin-top: 20px;">
                            <label>Тип *</label>
                            <select name="type" class="form-control">
                                <option value="1" {{ $model->type == 1 ? ' selected=selected' : '' }}>Разозовая</option>
                                <option value="2" {{ $model->type == 2 ? ' selected=selected' : '' }}>Лимитированная</option>
                                <option value="3" {{ $model->type == 3 ? ' selected=selected' : '' }}>Бесконечная</option>
                            </select>
                        </div>
                        <div class="group" style="margin-top: 20px;">
                            <label>Кол-во раз в задаче</label>
                            <input name="count" class="form-control" value="{{$model->count}}">
                        </div>
                        <div class="group" style="margin-top: 20px;">
                            <label>Инструкция</label>
                            <textarea name="instruction" class="form-control" rows="10">{{$model->instruction}}</textarea>
                        </div>
...
<script>
    ...
    $('.js-edit').click(function (e) {
        ...
        e.stopPropagation();
        $.ajax({
            url: '/dashboard/airdrop-task-edit-post',
            data: {
                id: {{$model->id}},
